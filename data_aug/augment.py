...
def gen_eda(train_orig, output_file, alpha, num_aug=9):
    ...
    for i, line in tqdm(enumerate(lines)):
        ...
        query = parts[1]
        answer=parts[2]
        aug_queries = eda(query, alpha_sr=alpha, alpha_ri=alpha, alpha_rs=alpha, p_rd=alpha, num_aug=num_aug)
        aug_answers = eda(answer, alpha_sr=alpha, alpha_ri=alpha, alpha_rs=alpha, p_rd=alpha, num_aug=num_aug)
        for aug_query in aug_queries:
            for aug_answer in aug_answers:
                writer.write(label + "\t" + aug_query +"\t"+aug_answer+'\n')
